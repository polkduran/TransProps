...
namespace TransProp.Core
{
  ...
  */
    ...
    /// <summary>
    /// An exception thrown by <see cref="PropsReader"/> when parsing
    /// a properties stream.
    /// </summary>
    ...
    {
        public PropsElement(string key, string value)
        {
            this.Key = key;
            this.Value = value;
            if (Key == "#" || Key == "!")
    ...
    public class PropsReaderFileReader : PropsReader
    {
    ...
    /// </summary>
    public class PropsReader : IObservable<PropsElement>
    {
        ...
        private static int[][] states = new int[][] {
			new int[]{//STATE_start
				...
				'#',				STATE_comment,			ACTION_add_to_key,
				'!',				STATE_comment,			ACTION_add_to_key,
            ...
            new int[]{//STATE_comment
				...
				MATCH_any,			STATE_comment,			ACTION_add_to_value,
			},
			new int[]{//STATE_key
				MATCH_end_of_input,	STATE_finish,			ACTION_store_property,
				MATCH_terminator,	STATE_start,			ACTION_store_property,
        ...
        private void doAction(int action, int ch)
        {
            switch (action)
            {
                ...
                case ACTION_store_property:
                    ...
                    PropsElement element = new PropsElement(key, value);
                    ObserversOnNext(element);
        ...
        private int peekChar()
        {
            ...
            saved = true;
            return savedChar = Read();
        }


        private List<IObserver<PropsElement>> observers;
        public void Subscribe(IObserver<PropsElement> observer)
        {
            if (observers == null)
            {
                observers = new List<IObserver<PropsElement>>();
            }
            observers.Add(observer);
        }

        private void ObserversOnNext(PropsElement element)
        {
            if (observers != null)
            {
                foreach (IObserver<PropsElement> observer in observers)
                {
        ...
        private void ObserversOnError(Exception ex)
        {
            if (observers != null)
            {
                foreach (IObserver<PropsElement> observer in observers)
        ...
        private void ObserversOnComplete()
        {
            if (observers != null)
            {
                foreach (IObserver<PropsElement> observer in observers)

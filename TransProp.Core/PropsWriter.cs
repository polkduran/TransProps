...
namespace TransProp.Core
{
    ...
    public class PropsWriter
    {
        ...
        public void Write(Stream stream, IEnumerable<PropsElement> elements)
        {
            ...
            foreach (PropsElement element in elements)
            {
                ...
                string key = escapeKey(element.Key);
                string value = escapeValue(element.Value);

                string keyValue = string.Format("{0}={1}",key,value);

                writer.WriteLine(keyValue);
                count++;
                if (count % 100 == 0)
                {
                    //todo : evil monkey will come for you, but it works
                    writer.Flush();
                }
            }
